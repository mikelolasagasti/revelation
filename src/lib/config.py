...
# $Id: data.py 169 2004-10-23 22:27:37Z erikg $
#
...
from revelation import io, util
import revelation
...
class Config(object):
	...
	def __init__(self, basedir = "/apps/revelation"):
		...
		if self.check() == False:
			if self.__install_schema(revelation.DIR_GCONFSCHEMAS + "/revelation.schemas") == False or self.check() == False:
				raise ConfigError
	...
	def __install_schema(self, file):
		...
		util.execute(revelation.FILE_GCONFTOOL + " --install-schema-file=" + file)
